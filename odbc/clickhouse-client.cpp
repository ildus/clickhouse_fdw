 ...
 * */
int
odbc_error(unsigned int type, const SQLHANDLE& handle, char *error)
{
 ...
 * */ 
Conn*
odbc_connect(char *driver, char *host, int port, char *dbname,
					char *user, char *pass, char *error)
{
		Conn *conn = new Conn();		
		char url[512] = {0};
    ...
    SQLSMALLINT retlen = 1024;
		...
		error[0] = '\0';
    if(SQL_SUCCESS!=SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &conn->henv))
		{
			delete conn;
			return NULL;
		}
    
		if(SQL_SUCCESS!=SQLSetEnvAttr(conn->henv,SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3, 0))
		{
			delete conn;
			return NULL;
		}
 ...
 * */
int
odbc_prepare(Conn *conn, char *query)
{
	SQLRETURN rc;
 ...
 * */
const char*
odbc_get_string(Conn *conn, int i, char* v, int len, bool *is_null)
{
	SQLLEN l = 0;		
 ...
 * */
int
odbc_get_int(Conn *conn, int i, int *erno)
{
	int v;
	*erno = 0;	
 ...
 * */
int
odbc_get_date(Conn *conn, int i, char *v)
{
 ...
 * */
int
odbc_bind_string(Conn *conn, int i, char *v, bool is_null)
{
	int l = strlen(v);
	SQLRETURN rc = SQLBindParameter(conn->stmt, i, SQL_PARAM_INPUT, SQL_CHAR, SQL_LONGVARCHAR, l, 0, v, 0, NULL);
	if(rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO)
		return 0;
	odbc_error(SQL_HANDLE_STMT, conn->stmt, conn->error);
	(void) is_null;
 ...
 * */
int
odbc_bind_int(Conn *conn, int i, int v, bool is_null)
{
	SQLRETURN rc = SQLBindParameter(conn->stmt, i, SQL_PARAM_INPUT, SQL_C_ULONG, SQL_INTEGER, 5, 0, &v, 0, NULL);
	if(rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO)
		return 0;
	odbc_error(SQL_HANDLE_STMT, conn->stmt, conn->error);
	(void) is_null;
...
/*
 * Get the column count from the currently parapred query from the satatment handle.
 * */
int
odbc_column_count(Conn* conn)
{
	int c;
